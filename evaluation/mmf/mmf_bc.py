...
import random
import pickle
from train_bc import train_model_without_subset, test_model_with_subset 
...
def create_flipped_label(data_dir, flip_ratio):
    ...
    z = {**x, **y}
    outfile = './flipped_label_partition_bc.pkl'
    f = open(outfile, "wb")
    pickle.dump(z, f)
    return z 
...
def identify_noisy_data(data_dir, num_partition, num_subset):
    ...
    training_list_A = ['scat.' + str(e) + '.jpg' for e in range(1, 1000, 1)]
    training_list_D = ['hetro.' + str(e) + '.jpg' for e in range(1, 1000, 1)]
