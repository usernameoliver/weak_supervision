...
import random
import pickle
from train_ddsm import train_model_without_subset, test_model_with_subset 
 

def create_flipped_label(data_dir, flip_ratio):
    path1 = data_dir + 'breastcancer/'
    path2 = data_dir + 'breastnormal/'
    ...
    z = {**x, **y}
    outfile = './flipped_label_partition_ddsm.pkl'
    f = open(outfile, "wb")
    pickle.dump(z, f)
    return z 
...
def identify_noisy_data(data_dir, num_partition, num_subset):
    ...
    training_list_A = ['cancer_MLO.' + str(e) + '.png' for e in range(0, 400, 1)]
    training_list_D = ['normal_MLO.' + str(e) + '.png' for e in range(0, 400, 1)]
